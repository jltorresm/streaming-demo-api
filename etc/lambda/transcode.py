...
"""
This is lambda function is based on AWS samples here:
...
It has slight modifications to adapt it to this demo, mainly simplify it.

The current job.json file was built based on the generated json from the MediaConvert console. The full job spec can
be found here:
...
def handler(event, context):
    ...
    destinationS3 = 's3://' + sourceS3Bucket + "/transcoded/"
    ...
    try:
        ...
        for j in jobs:
            ...
            #
            # NOTE: Here I modified the generic logic to simplify it and fit my demo
            #
            for outputGroup in jobSettings['OutputGroups']:
                ...
                if outputGroup['OutputGroupSettings']['Type'] == 'HLS_GROUP_SETTINGS':
                    logger.info("transcoded file destination: %s", destinationS3)
                    outputGroup['OutputGroupSettings']['HlsGroupSettings']['Destination'] = destinationS3

                else:
                    logger.error("Exception: Unexpected Output Group Type %s",
                                 outputGroup['OutputGroupSettings']['Type'])
                    statusCode = 500
